...
function getThemeCluster(theme, type = '') {
  ...
  if (type && type === 'new' && clusters.length) {
    ...
    // console.log('--theme-weaken-color-10', clusters[10])
    // document.body.style.setProperty('--theme-weaken-color-10', clusters[10]) // 一般是 hover 是的颜色
    // document.body.style.setProperty('--theme-weaken-color-6', clusters[6])
  }
  return clusters
}
 ...
 * main
 */
import ElementPlusCSS from './elementPlusCssStr'
export const updateThemeColor = function (val) {
  document.body.style.setProperty(`--theme-color`, val) // 主题色
  if (typeof val !== 'string' || val.length === 0) return
  const ORIGINAL_THEME = '#409EFF' // default color (element ui的默认主题色，所有我们根据这个去改)
  const ThemeCode = ElementPlusCSS.replace(/@font-face{[^}]+}/g, '') // 去掉字体样式
